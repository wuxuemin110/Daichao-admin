...
<script>
  ...
  export default {
    ...
    methods: {
      ...
      Download () {
        window.location.href = this.$http.adornUrl(`/sys/report/rechargeReportDownload?token=${this.$cookie.get('token')}${this.pageIndex ? '&page=' + this.pageIndex : ''}${this.pageSize ? '&limit=' + this.pageSize : ''}${this.dataForm.productName ? '&productName=' + this.dataForm.productName : ''}${this.dataForm.productDisplayNum ? '&productDisplayNum=' + this.dataForm.productDisplayNum : ''}${this.dataForm.startDate ? '&startDate=' + this.dataForm.startDate : ''}${this.dataForm.endDate ? '&endDate=' + this.dataForm.endDate : ''}`)
      },
      //获取合计
      getALLdataList(){
        this.$http({
