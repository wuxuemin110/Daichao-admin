  ...
  <el-dialog
    ...
    <el-form :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="dataFormSubmit()" label-width="80px">
      ...
      <el-form-item label="销售员">
        <el-select v-model="dataForm.saleUserId" placeholder="请选择销售员" style="width: 200px;">
          <el-option
            v-for="item in sales"
            :key="item.userId"
            :label="item.realName"
  ...
  export default {
    ...
    methods: {
          ...
          if (this.dataForm.id) {
            this.$http({
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.saleUserId = data.channel.saleUserId
      ...
      getSales () {
        this.sales = []
        this.$http({
          url: this.$http.adornUrl(`/sys/channel/sales`),
          method: 'get',
          params: this.$http.adornParams()
        }).then(({data}) => {
          if (data && data.code === 0) {
            if (data.page !== undefined) {
              this.sales = this.sales.concat(data.page)
            }
          }
        })
      },
